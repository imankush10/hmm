...
export default function MaterialSankey() {
  // Find the specific data points needed for calculations
  const processing =
    sankeyData.find((d) => d.name === "Processing")?.value || 0;
  const recycled =
    sankeyData.find((d) => d.name === "Recycled Materials")?.value || 0;
  const products = sankeyData.find((d) => d.name === "Products")?.value || 0;
  const manufacturing =
    sankeyData.find((d) => d.name === "Manufacturing")?.value || 0;
  const waste = sankeyData.find((d) => d.name === "Waste")?.value || 0;

  // Calculate the metrics
  const processEfficiency = processing > 0 ? (products / processing) * 100 : 0;
  const recycledContent = processing > 0 ? (recycled / processing) * 100 : 0;
  const wasteRate = manufacturing > 0 ? (waste / manufacturing) * 100 : 0;
